...
class WorldCupPredictionInline(admin.TabularInline):
    model = WorldCupPrediction
    extra = 0
    readonly_fields = ('user', 'home_score', 'away_score', 'points_earned', 'created_at')
    can_delete = False
...
class WorldCupMatchAdmin(admin.ModelAdmin):
    list_display = (
        'home_team', 'away_team', 'kickoff_at', 'stage', 'group_name',
        'home_score', 'away_score', 'status', 'predictions_open',
    )
    list_filter = ('status', 'stage', 'predictions_open', 'kickoff_at')
    search_fields = ('home_team', 'away_team', 'group_name')
    readonly_fields = ('created_at', 'updated_at')
    inlines = [WorldCupPredictionInline]
    list_per_page = 25
...
@admin.register(WorldCupPrediction)
class WorldCupPredictionAdmin(admin.ModelAdmin):
